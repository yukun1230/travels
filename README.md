...
- `React Native` + `Expo` + `Redux` + `React Navigation` + `Axios` + `React Native Paper`

#### PC端

- `React` + `Ant Design` + `Redux` + `Axios`
...
1. **喻昆：**
   ...
   - 后端服务的搭建和维护工作，设计和优化后端数据库结构，确保数据存储和查询的高效性和稳定性，为核心功能的实现提供服务端支持。
   - 移动端App登录页面、注册页面、用户信息更改页面、游记发布页面、游记编辑页面、分享内容承接页的搭建和功能逻辑的实现。处理了表单提交以及多图上传逻辑。
   - 将整个移动端项目打包为`.apk`文件，使其能够安装在安卓手机上。
   - 实现后端服务与用户移动端 App 和 PC 网页站点审核系统的接口，确保数据的安全传输和业务逻辑的正确执行。

2. **谌翔：**

   - 移动端App用户系统的开发工作，主要包括首页、游记详情页、我的游记与我的收藏页面的搭建；以及首页搜索、刷新、加载；详情页点赞、收藏、分享；我的游记与我的收藏页面的游记管理逻辑功能的实现。
   - 协助后端设计和调试 API 接口，确保移动端 App 能够与后端服务正常交互。
   - 负责移动端 App 的 UI/UX 设计，提供用户友好的界面和流畅的操作体验。

3. **杨婷婷：**

   - PC 网页站点审核系统的开发和管理工作，包括管理员登录、游记审核、数据统计等模块的实现。
